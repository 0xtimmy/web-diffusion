...
export class Module {
    ...
    private *_named_members<T>(
        ...
        const modules: Generator<[string|number, Module]> | [string|number, Module][] =
            recurse
                ...
                : [[prefix, this]];
        for (let [module_prefix, module] of modules) {
    ...
    *buffers(): Generator<Tensor | null> {
     ...
     * @returns this module
     */
    train(mode = true): ThisType<Module> {
        this._training = mode;
     ...
     * @returns this module
     */
    zeroGrad(setToNull = true): ThisType<Module> {
        for (const parameter of this.parameters()) {
            if (parameter.grad) {
                if (setToNull) {
                    parameter.grad = null;
    ...
    private async _loadStateDictFromURL(dirname: string, prefix: string): Promise<void> {
        for(const [name, old] of this.immediateParameters) {
            const completeName = prefix + name;
            const buf = await (await fetch(`${dirname}/${completeName}/data`)).arrayBuffer();
...
export class Parameter extends Tensor {
    constructor(data: Tensor, requiresGrad = true) {
        //data = data; // || empty(0);
        super({
            data: data.storage,
            dtype: data.dtype,
