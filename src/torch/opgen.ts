...
import { BinaryOpSpec, ScalarOpSpec, OpSpec, ReductionOpSpec, UnaryOpSpec } from "./op_spec";
...
export function opSpecToKernelSpecs(op: OpSpec): KernelSpec[] {
    if (op.type == "reduction") {
        ...
        return getScalarKernelSpecs(op as ScalarOpSpec);
    } else {
...
function getScalarKernelSpecs(op: ScalarOpSpec): KernelSpec[] {
    const specs = [getScalarKernelSpec(op), getScalarInplaceKernelSpec(op)];
    if (op.backward) {
        specs.push(getScalarGradKernelSpec(op, op.backward));
    }
    return specs;
}
...
function getScalarKernelSpec(op: ScalarOpSpec): KernelSpec {
...
function getScalarInplaceKernelSpec(op: ScalarOpSpec): KernelSpec {
    const parameters: KernelParamSpec[] = [
        {
            name: "size",
            shaderType: "u32",
...
function getScalarGradKernelSpec(
    op: ScalarOpSpec,
    backward: ExprCode
): KernelSpec {
    const parameters: KernelParamSpec[] = [
