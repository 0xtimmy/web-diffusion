...
class Diffusion {
    ...
    prepare_noise_schedule() {
        return linspace(this.beta_start, this.beta_end, this.noise_steps);
    }

    async calc_alpha_hat() {
        this.alpha_hat = await cumprod(this.alpha, 0);
    }

    sample_timesteps(n: number) {
        return randint(1, this.noise_steps, n);
    }

    sample(model, n=1) {
        console.log(`Sampling ${n} new images...`);
        model.eval();
        let x = randn([n, 3, this.img_size, this.img_size]);
        for(let i = this.noise_steps; i >= 0; i--) {
            const t = torch.mul(torch.ones(n), torch.ones(n), i);
            const predicted_noise = model(x, t);
        ...
        model.train();
        x = torch.scalar_div(torch.scalar_add(clamp(x, -1, 1), 1), 2);
